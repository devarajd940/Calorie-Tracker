...
class GeminiAnalyzer {
    ...
    suspend fun analyzeFood(imagePath: String): FoodAnalysisResult {
        try {
            ...
            val inputContent = content {
                image(bitmap)  // Send Bitmap, not ByteArray
                text(prompt)
            }
    ...
    private fun parseResponse(response: String): FoodAnalysisResult {
        try {
            ...
            return FoodAnalysisResult(foodName, calories, details)

        } catch (e: Exception) {
            return FoodAnalysisResult("Unknown Food", 300, "Could not parse AI response")
        }
    }
}
